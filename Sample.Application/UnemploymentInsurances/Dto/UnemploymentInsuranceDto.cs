...
namespace Sample.Application.UnemploymentInsurances.Dto
{
    public class UnemploymentInsuranceDto : AuditingDto
    {
        ...
        public List<string?> AttachmentFiles => !string.IsNullOrEmpty(AttachmentFile) ? AttachmentFile.Split('|').Select(x => (string?)x).ToList() : new List<string?>();
